# Installing Dawnlight on iOS via iloader

## Prerequisites

- A Windows, Linux, or macOS device
- iOS device connected to computer via USB
- Dawnlight IPA file (download the latest `Dawnlight-vX.X.X-ios-arm64.ipa` from the [releases page](https://github.com/BeZide93/dusk/releases))
...
## 3. Install Dawnlight on Your iPhone
...
3. Leave settings unchanged (the Anisette server should stay Sidestore (.io))
    3.(a) Installing SideStore directly is not required, but provides you a way to install Dawnlight on your phone without being plugged into a computer later
4. Press `Import IPA` and choose your downloaded `Dawnlight-v.X.X.X-ios-arm64.ipa`, it will begin installing on your device

**NOTE:** *At various stages, you may be prompted to trust your device, do so*

## 3. Getting Dawnlight trusted
...
Transfer the game disc (and optionally, the Dawnlight IPA) to your iPhone so they are accessible in the Files app. A few ways to do this:
...
You may now use Dawnlight on iOS and iPadOS!
