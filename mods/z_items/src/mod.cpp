...
HookAction before_ring_set_active_cursor(ModContext*, void* args, void*, void*) {
    ...
    if (ring == nullptr || !mDoCPd_c::getTrigZ(PAD_1)) {
        return HOOK_CONTINUE;
    }
...
ModResult add_hook(ModResult result, ModError* error) {
    return result == MOD_OK ? MOD_OK :
        mods::set_error(error, result, "failed to install Z item hooks");
}

ModResult install_z_item_hooks(ModError* error) {
    ...
    if (result == MOD_OK) {
        result = mods::hook_add_post<SetStickDataHook>(svc_hook, after_set_stick_data);
    }
    if (result == MOD_OK) {
...
MOD_EXPORT ModResult mod_initialize(ModError* error) {
    if (const ModResult result = install_z_item_hooks(error); result != MOD_OK) {
        return result;
    }

    svc_log->info(mod_ctx, "Z item mod initialized");
    return MOD_OK;
}
...
MOD_EXPORT ModResult mod_shutdown(ModError*) {
    svc_log->info(mod_ctx, "Z item mod unloaded");
    return MOD_OK;
}
