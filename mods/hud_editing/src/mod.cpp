...
void window_closed(ModContext*, UiWindowHandle, void*) {
    s_window = 0;
}

void open_settings(ModContext*, void*) {
    if (s_window != 0) {
        return;
    }
    ...
    tabs[0].title = "Layout";
    tabs[0].build = build_layout_tab;
    tabs[1].title = "JSON";
    tabs[1].build = build_files_tab;

    UiWindowDesc desc = UI_WINDOW_DESC_INIT;
    desc.tabs = tabs.data();
    desc.tab_count = tabs.size();
    desc.on_closed = window_closed;
    svc_ui->window_push(mod_ctx, &desc, &s_window);
}

ModResult build_mod_panel(ModContext*, UiElementHandle panel, void*, ModError*) {
    if (add_section(panel, "Dawnlight HUD Editing") != MOD_OK) return MOD_ERROR;
    if (add_button(panel, "Open HUD Editing Settings", open_settings) != MOD_OK) {
        return MOD_ERROR;
    }
    return MOD_OK;
}
...
ModResult register_ui(ModError* error) {
    ...
    if (result != MOD_OK) {
        return set_error(error, result, "failed to register HUD editing mod panel");
    }
    ...
    if (result != MOD_OK) {
        return set_error(error, result, "failed to register HUD editing menu tab");
    }
    return MOD_OK;
}
...
MOD_EXPORT ModResult mod_initialize(ModError* error) {
    if (const ModResult result = register_config(error); result != MOD_OK) {
        return result;
    }
    if (const ModResult result = register_ui(error); result != MOD_OK) {
        return result;
    }
    if (const ModResult result = register_hud_layout(error); result != MOD_OK) {
        return result;
    }
...
MOD_EXPORT ModResult mod_shutdown(ModError*) {
    svc_log->info(mod_ctx, "Dawnlight HUD Editing unloaded");
    return MOD_OK;
}
