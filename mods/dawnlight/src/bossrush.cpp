...
class BossRushReserve {
public:
    explicit BossRushReserve(dSv_save_c* save) : mBytes(reinterpret_cast<u8*>(save) + 0x8F0) {}

    bool isBossRush() const {
        static constexpr char magic[] = "DUSKBR1";
        return std::memcmp(mBytes + 32, magic, sizeof(magic) - 1) == 0;
    }
    void setBossRush(bool enabled) {
        static constexpr char magic[] = "DUSKBR1";
        if (enabled) {
            std::memcpy(mBytes + 32, magic, sizeof(magic) - 1);
        } else {
            std::memset(mBytes + 32, 0, 16);
        }
    }
    u8 getBossRushIndex() const { return isBossRush() ? mBytes[40] : 0; }
    void setBossRushIndex(u8 value) { if (isBossRush()) mBytes[40] = value; }
    u8 getBossRushLoop() const { return isBossRush() ? mBytes[41] : 0; }
    void setBossRushLoop(u8 value) { if (isBossRush()) mBytes[41] = value; }
    u8 getBossRushState() const { return isBossRush() ? mBytes[42] : 0; }
    void setBossRushState(u8 value) { if (isBossRush()) mBytes[42] = value; }

private:
    u8* mBytes;
};

BossRushReserve reserve() {
    return BossRushReserve(dComIfGs_getSaveData());
}
...
bool can_offer_midna_hub_warp() {
    return reserve().isBossRush() && !is_boss_hub_stage_name() && !has_hub_midna_prompt() &&
           can_hub_warp();
}
...
void apply_new_save_preset() {
    BossRushReserve saveReserve = reserve();
