...
ModResult build_hud_tab(
    ...
    return build_hud_layout_settings_tab(ctx, window, left, right, userData, error);
}

ModResult build_hud_json_tab(
    ModContext* ctx, UiWindowHandle window, UiElementHandle left, UiElementHandle right,
    void* userData, ModError* error) {
    return build_hud_layout_files_tab(ctx, window, left, right, userData, error);
}
...
void open_settings(ModContext* ctx, void*) {
    ...
    tabs[4].title = "HUD JSON";
    tabs[4].build = build_hud_json_tab;
