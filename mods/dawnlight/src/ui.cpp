...
ModResult build_controls_tab(
    ...
    if (add_toggle(ctx, left, "Z Item Slot", z_item_slot_config_var(),
            "Enables Dawnlight's logical Z item slot for saves and item use where the slot is "
            "already assigned. Full item-wheel assignment, Wii U controller relabeling, and HUD "
            "button rendering still need upstream host services.")
        != MOD_OK)
    {
        return MOD_ERROR;
    }
...
ModResult build_hud_tab(
    ModContext* ctx, UiWindowHandle, UiElementHandle left, UiElementHandle, void*, ModError*) {
    if (add_section(ctx, left, "HUD Layout") != MOD_OK) return MOD_ERROR;
    if (add_text(ctx, left,
            "The exported hud_layout_settings.json format is kept compatible with Dawnlight, but "
            "this host-clean .dusk cannot apply HUD pane transforms until upstream exposes a "
            "stable HUD layout or meter-pane service.")
        != MOD_OK)
    {
        return MOD_ERROR;
    }
    if (add_text(ctx, left,
            "The current Mod API does not expose the host settings pages or a file picker yet, so "
            "the visual HUD editor remains outside this .dusk package for now.")
        != MOD_OK)
    {
        return MOD_ERROR;
    }
