...
bool RestartProcess(int argc, char* argv[]) {
    ...
    {
        fprintf(stderr, "Failed to restart Dawnlight: CreateProcessW error %lu\n", GetLastError());
        return false;
    }
    ...
    if (executablePath.empty()) {
        fprintf(stderr, "Failed to restart Dawnlight: unable to resolve executable path\n");
        return false;
    }
    ...
    fprintf(stderr, "Failed to restart Dawnlight: execv failed: %s\n", std::strerror(errno));
    return false;
#endif
}
