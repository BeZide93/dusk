...
void set_control_label(Rml::Element* root, const char* label) noexcept {
    if (root == nullptr) {
        return;
    }
    if (auto* span = root->QuerySelector("span")) {
        span->SetInnerRML(label);
    }
}
...
void TouchControls::sync_visual_state() noexcept {
    if (mWasSuppressed || !getSettings().game.enableTouchControls) {
        clear_motion_touch_input();
        for (const auto control : {Control::L, Control::ZL, Control::R, Control::MINUS}) {
    ...
    const bool wiiuStyle = UseWiiUControllerStyle();
    ...
    if (zlTrigger.root != nullptr) {
        ...
        zlTrigger.root->SetPseudoClass("hidden", zlHidden);
    }
    if (rTrigger.root != nullptr) {
        set_control_label(rTrigger.root, wiiuStyle ? "ZR" : "R");
        rTrigger.root->SetPseudoClass("hidden", rHidden);
    }
    if (minusButton.root != nullptr) {
...
void TouchControls::sync_control_displays() noexcept {
    ...
    if (z.root != nullptr) {
        set_control_label(z.root, UseWiiUControllerStyle() ? "R" : "Z");
