...
constexpr SDL_DialogFileFilter kGciSaveFilters[] = {
    {"GameCube Save", "gci"},
    {"All Files", "*"},
};
...
void push_save_toast(Rml::String title, Rml::String content) noexcept {
    push_toast({
        .title = std::move(title),
        .content = std::move(content),
        .duration = std::chrono::seconds(4),
...
struct LayoutExportState {
    Rml::String label;
    std::string text;
};

std::string save_dialog_default_location(const char* fileName) {
    if (ConfigPath.empty()) {
        return fileName;
    }
    return io::fs_path_to_string(ConfigPath / fileName);
}

void export_layout_file_callback(void* userdata, const char* path, const char* error) {
    std::unique_ptr<LayoutExportState> state(static_cast<LayoutExportState*>(userdata));
    if (!state) {
        return;
    }
    if (error != nullptr) {
        push_layout_toast(state->label + " Export Failed", escape(error));
        return;
    }
    ...
    try {
        write_file_select_text(path, state->text);
        push_layout_toast(state->label + " Exported", escape(display_name_for_path(path)));
    } catch (const std::exception& error) {
        push_layout_toast(state->label + " Export Failed", escape(error.what()));
    }
}

void open_layout_export_picker(const char* fileName, const json& layout, Rml::String label) {
    static constexpr SDL_DialogFileFilter filters[] = {
        {"Dusk Layout", "json"},
        {"All Files", "*"},
    };
    static std::string defaultLocation;
    defaultLocation = save_dialog_default_location(fileName);

    auto state = std::make_unique<LayoutExportState>();
    state->label = std::move(label);
    state->text = layout.dump(4);

    ShowFileSave(export_layout_file_callback, state.release(), nullptr, filters,
        static_cast<int>(std::size(filters)), defaultLocation.c_str());
}

void import_hud_layout_file_callback(void* userdata, const char* path, const char* error) {
    (void)userdata;
    const Rml::String label = "HUD Layout";
    if (error != nullptr) {
    ...
    try {
        const json root = json::parse(read_file_select_text(path));
        if (!import_hud_layout_json(root)) {
            push_layout_toast(
...
void open_hud_layout_import_picker() {
    ...
    ShowFileSelect(import_hud_layout_file_callback, nullptr, nullptr, filters,
        static_cast<int>(std::size(filters)),
        defaultLocation.empty() ? nullptr : defaultLocation.c_str(), false);
}

void export_hud_layout_file() {
    open_layout_export_picker("hud_layout_settings.json", export_hud_layout_json(), "HUD Layout");
}
...
struct SaveImportState {
    SaveFileInfo info;
};

const char* configured_save_region() {
    return prelaunch_state().configuredDiscInfo.isPal ? "EUR" : "USA";
}

const char* configured_save_game_id() {
    return prelaunch_state().configuredDiscInfo.isPal ? "GZ2P" : "GZ2E";
}

SaveFileInfo current_save_file_info() {
    const std::string region = configured_save_region();
    const int type = getSettings().backend.cardFileType.getValue();

    if (type == kCardRawImageType) {
        std::string fileName = "MemoryCardA." + region + ".raw";
        return {
            .path = ConfigPath / fileName,
            .defaultFileName = std::move(fileName),
            .label = "Card Image Save",
            .filters = kCardImageSaveFilters,
            .filterCount = static_cast<int>(std::size(kCardImageSaveFilters)),
        };
    }

    std::string fileName =
        "01-" + std::string(configured_save_game_id()) + "-gczelda2.gci";
    return {
        .path = ConfigPath / region / "Card A" / fileName,
        .defaultFileName = std::move(fileName),
        .label = "GCI Save",
        .filters = kGciSaveFilters,
        .filterCount = static_cast<int>(std::size(kGciSaveFilters)),
    };
}

std::string save_export_default_location(const SaveFileInfo& info) {
    if (ConfigPath.empty()) {
        return info.defaultFileName;
    }
    return io::fs_path_to_string(ConfigPath / info.defaultFileName);
}

void export_save_file_callback(void* userdata, const char* path, const char* error) {
    std::unique_ptr<SaveExportState> state(static_cast<SaveExportState*>(userdata));
    if (!state) {
        return;
    }
    if (error != nullptr) {
        push_save_toast(state->label + " Export Failed", escape(error));
        return;
    }
    if (path == nullptr || path[0] == '\0') {
        return;
    ...
    try {
        write_file_select_text(path, state->data);
        push_save_toast(state->label + " Exported", escape(display_name_for_path(path)));
    } catch (const std::exception& error) {
        push_save_toast(state->label + " Export Failed", escape(error.what()));
    }
}

void open_save_export_picker() {
    if (ConfigPath.empty()) {
        push_save_toast("Save Export Failed", "No config directory is available.");
        return;
    }

    const SaveFileInfo info = current_save_file_info();
    if (!std::filesystem::exists(info.path)) {
        push_save_toast(info.label + " Export Failed", "No save file exists yet.");
        return;
    }

    auto state = std::make_unique<SaveExportState>();
    state->label = info.label;
    try {
        const auto data = io::FileStream::ReadAllBytes(info.path);
        state->data.assign(reinterpret_cast<const char*>(data.data()), data.size());
    } catch (const std::exception& error) {
        push_save_toast(info.label + " Export Failed", escape(error.what()));
        return;
    }

    static std::string defaultLocation;
    defaultLocation = save_export_default_location(info);
    ShowFileSave(export_save_file_callback, state.release(), nullptr, info.filters,
        info.filterCount, defaultLocation.c_str());
}

void import_save_file_callback(void* userdata, const char* path, const char* error) {
    std::unique_ptr<SaveImportState> state(static_cast<SaveImportState*>(userdata));
    if (!state) {
        return;
    }
    if (error != nullptr) {
        push_save_toast(state->info.label + " Import Failed", escape(error));
        return;
    }
    if (path == nullptr || path[0] == '\0') {
        return;
    ...
    try {
        const std::string data = read_file_select_text(path);
        if (data.empty()) {
            push_save_toast(state->info.label + " Import Failed", "The selected file is empty.");
            return;
        }

        std::filesystem::create_directories(state->info.path.parent_path());
        auto file = io::FileStream::Create(state->info.path);
        file.Write(data.data(), data.size());
        push_save_toast(state->info.label + " Imported", escape(display_name_for_path(path)));
    } catch (const std::exception& error) {
        push_save_toast(state->info.label + " Import Failed", escape(error.what()));
    }
}

void open_save_import_picker() {
    if (ConfigPath.empty()) {
        push_save_toast("Save Import Failed", "No config directory is available.");
        return;
    }

    auto state = std::make_unique<SaveImportState>();
    state->info = current_save_file_info();

    static std::string defaultLocation;
    defaultLocation = io::fs_path_to_string(ConfigPath);
    const SDL_DialogFileFilter* filters = state->info.filters;
    const int filterCount = state->info.filterCount;
    ShowFileSelect(import_save_file_callback, state.release(), nullptr, filters,
        filterCount, defaultLocation.c_str(), false);
}
...
SettingsWindow::SettingsWindow(bool prelaunch) : mPrelaunch(prelaunch) {
        ...
        add_tab("Prelaunch", [this](Rml::Element* content) {
            ...
            leftPane.register_control(
                ...
                rightPane, [](Pane& pane) {
                    ...
                    pane.add_rml("<br/>");
                    pane.add_button("Export Save").on_pressed([] { open_save_export_picker(); });
                    pane.add_button("Import Save").on_pressed([] { open_save_import_picker(); });
                    pane.add_rml("<br/>Exports or imports the Slot A save file for the selected "
                                 "save file type.");
                });
        });
    }
    ...
    add_tab("Input", [this](Rml::Element* content) {
        ...
        addOption("Aim Movement", getSettings().game.enableAimMovement,
            "Allows movement while aiming supported items.<br/><br/>Supported items include the "
