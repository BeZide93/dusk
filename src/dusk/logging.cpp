...
bool is_generated_log_file_name(const std::filesystem::path& path) {
    ...
    constexpr std::string_view currentPrefix = "dawnlight-"sv;
    constexpr std::string_view legacyDusklightPrefix = "dusklight-"sv;
    ...
    } else if (filename.starts_with(legacyDusklightPrefix)) {
        timestampOffset = legacyDusklightPrefix.size();
...
std::string MakeTimestampedLogName() {
    ...
    std::strftime(buffer.data(), buffer.size(), "dawnlight-%Y%m%d-%H%M%S.log", &localTime);
    return buffer.data();
}
