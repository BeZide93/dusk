...
f32 hud_button_background_alpha(f32 alpha) {
    if (!dusk::getSettings().game.hudButtonBackground.getValue() ||
        hud_button_background_hidden_by_context())
    {
        return 0.0f;
    }
...
bool use_round_xy_hud_buttons() {
    return dusk::getSettings().game.hudRoundXYButtons.getValue();
}
...
void scale_xy_hud_button_base(CPaneMgr* button, f32 scale) {
    if (use_round_xy_hud_buttons()) {
        scale_round_hud_button_base(button, scale);
    } else {
        button->scale(scale, scale);
    }
}

struct HudItemAnchorOffset {
    f32 x = 0.0f;
    f32 y = 0.0f;
};
...
HudItemAnchorOffset hud_item_anchor_position(
    dusk::hud_layout::ItemAnchor anchor, f32 defaultX, f32 defaultY) {
    const f32 distance = abs_f32(defaultX) > 14.0f ? abs_f32(defaultX) : 14.0f;

    switch (anchor) {
    case dusk::hud_layout::ItemAnchor::Left:
        return {-distance, defaultY};
    case dusk::hud_layout::ItemAnchor::Right:
        return {distance, defaultY};
    case dusk::hud_layout::ItemAnchor::Top:
        return {0.0f, defaultY - distance};
    case dusk::hud_layout::ItemAnchor::Bottom:
    default:
        return {0.0f, defaultY + distance};
    }
}

HudItemAnchorOffset hud_item_anchor_delta(
    dusk::hud_layout::Button button, f32 defaultX, f32 defaultY) {
    const HudItemAnchorOffset selected = hud_item_anchor_position(
        dusk::hud_layout::ButtonItemAnchor(button), defaultX, defaultY);
    const HudItemAnchorOffset fallback = hud_item_anchor_position(
        dusk::hud_layout::DefaultItemAnchor(button), defaultX, defaultY);
    return {
...
void apply_hud_item_anchor(
    dusk::hud_layout::Button button, f32 defaultX, f32 defaultY, f32& x, f32& y) {
    const HudItemAnchorOffset offset = hud_item_anchor_delta(button, defaultX, defaultY);
    x += offset.x;
    y += offset.y;
}

J2DTextBox* as_text_box(J2DPane* pane) {
    if (pane == NULL || pane->getTypeID() != 19) {
        return NULL;
    }

    return static_cast<J2DTextBox*>(pane);
}

J2DTextBoxHBinding hud_text_h_binding(dusk::hud_layout::SideAnchor anchor) {
    return anchor == dusk::hud_layout::SideAnchor::Right ? HBIND_LEFT : HBIND_RIGHT;
}

void set_hud_text_h_binding(J2DPane* pane, dusk::hud_layout::SideAnchor anchor) {
    if (pane == NULL) {
        return;
    }

    if (J2DTextBox* textBox = as_text_box(pane)) {
        textBox->mFlags = (textBox->mFlags & 0xf3) | (hud_text_h_binding(anchor) << 2);
        textBox->field_0x10c =
            anchor == dusk::hud_layout::SideAnchor::Right ? textBox->getWidth() : 0.0f;
    }

    for (J2DPane* child = pane->getFirstChildPane(); child != NULL;
         child = child->getNextChildPane()) {
        set_hud_text_h_binding(child, anchor);
    }
}

void set_hud_text_h_binding(CPaneMgr* textPane, dusk::hud_layout::SideAnchor anchor) {
    if (textPane == NULL) {
        return;
    }

    set_hud_text_h_binding(textPane->getPanePtr(), anchor);
}

f32 hud_text_anchor_offset(dusk::hud_layout::Button button, CPaneMgr* textPane) {
    const auto selected = dusk::hud_layout::ButtonTextAnchor(button);
    set_hud_text_h_binding(textPane, selected);
    return 0.0f;
}

dusk::hud_layout::Button hud_xy_button(int i_no) {
    return i_no == dMeter2Draw_c::SELECT_Y_e ? dusk::hud_layout::Button::Y :
                                               dusk::hud_layout::Button::X;
}

dusk::hud_layout::Button hud_item_button(int i_no) {
    return i_no == dMeter2Draw_c::SELECT_Z_e ? dusk::hud_layout::Button::Z :
                                               hud_xy_button(i_no);
}

HudItemAnchorOffset hud_button_item_offset(dusk::hud_layout::Button button) {
    return {
        .x = dusk::hud_layout::ButtonItemOffsetX(button),
        .y = dusk::hud_layout::ButtonItemOffsetY(button),
    };
}
...
void dMeter2Draw_c::draw() {
    ...
    for (int i = 0; i < SELECT_ITEM_MAX_e; i++) {
        ...
        if (itemPane != NULL) {
            const auto button = hud_item_button(i);
            ...
            const f32 ammoScale = hudTransform.scale *
                ...
                dusk::hud_layout::ButtonAmmoScale(button);
            const f32 ammoOffsetX = dusk::hud_layout::ButtonAmmoOffsetX(button);
            const f32 ammoOffsetY = dusk::hud_layout::ButtonAmmoOffsetY(button);
            for (int j = 0; j < 3; j++) {
                f32 temp_f30 = mItemParams[i].num_scale * 16.0f * ammoScale;

                Vec vtx0 = itemPane->getPanePtr()->getGlbVtx(0);
                Vec vtx3 = itemPane->getPanePtr()->getGlbVtx(3);

                mpItemNumTex[i][j]->draw(mItemParams[i].num_pos_x + ammoOffsetX +
                                             (((vtx0.x + vtx3.x) * 0.5f) + (temp_f30 * j)),
                                         mItemParams[i].num_pos_y + ammoOffsetY +
...
void dMeter2Draw_c::initButton() {
    ...
    for (int i = 0; i < 2; i++) {
        for (int j = 0; j < 2; j++) {
            ...
            mpItemRTex[i][j] = (ResTIMG*)heap->alloc(0xC00, 0x20);
            JUT_ASSERT(0, mpItemRTex[i][j] != NULL);
        }
    }
    ...
    if (dusk::UseWiiUControllerStyle()) {
        drawButtonZItem(dComIfGp_getSelectItem(2));
    } else {
...
void dMeter2Draw_c::drawLife(s16 i_maxLife, s16 i_life, f32 i_posX, f32 i_posY) {
    ...
    mpLifeParent->paneTrans(i_posX + hudTransform.offsetX, i_posY + hudTransform.offsetY);
}
...
void dMeter2Draw_c::drawOxygen(s32 i_max, s32 i_oxygen, f32 i_posX, f32 i_posY) {
    ...
    field_0x5cc[2] = g_drawHIO.mOxygenMeterScale * hudScale;
    field_0x5d8[2] = g_drawHIO.mOxygenMeterScale * hudScale;
...
void dMeter2Draw_c::drawRupee(s16 i_rupeeNum) {
    ...
    mpRupeeKeyParent->scale(g_drawHIO.mRupeeKeyScale * field_0x718 * hudScale,
                            g_drawHIO.mRupeeKeyScale * field_0x718 * hudScale);

    mpRupeeKeyParent->paneTrans(g_drawHIO.mRupeeKeyPosX + hudTransform.offsetX,
                                g_drawHIO.mRupeeKeyPosY + hudTransform.offsetY);
...
void dMeter2Draw_c::drawKey(s16 i_keyNum) {
    ...
    mpKeyParent->scale(g_drawHIO.mKeyScale * hudTransform.scale,
                       g_drawHIO.mKeyScale * hudTransform.scale);
    mpKeyParent->paneTrans(g_drawHIO.mKeyPosX + hudTransform.offsetX,
                           g_drawHIO.mKeyPosY + hudTransform.offsetY);
}
...
void dMeter2Draw_c::drawButtonA(u8 i_action, f32 i_posX, f32 i_posY, f32 i_textPosX, f32 i_textPosY,
    ...
    mpTextA->paneTrans(g_drawHIO.mButtonATextPosX + i_textPosX +
                           ...
                           textOffsetX + hudTransform.offsetX,
                       g_drawHIO.mButtonATextPosY + i_textPosY + textOffsetY +
...
void dMeter2Draw_c::drawButtonB(u8 i_action, bool param_1, f32 i_posX, f32 i_posY, f32 i_textPosX,
    ...
    const HudItemAnchorOffset itemAnchorOffset = hud_item_anchor_delta(
        dusk::hud_layout::Button::B, g_drawHIO.mButtonBItemPosX[var_r31],
        g_drawHIO.mButtonBItemPosY[var_r31]);
    const HudItemAnchorOffset itemOffset =
        hud_button_item_offset(dusk::hud_layout::Button::B);
    ...
    mpLightB->paneTrans(g_drawHIO.mItemBBasePosX[0] + i_posX + itemAnchorOffset.x +
                            itemOffset.x +
                            hudTransform.offsetX,
                        g_drawHIO.mItemBBasePosY[0] + i_posY + itemAnchorOffset.y +
                            itemOffset.y +
                            hudTransform.offsetY);
    ...
    mpTextB->paneTrans(g_drawHIO.mButtonBFontPosX + i_textPosX +
                           hud_text_anchor_offset(dusk::hud_layout::Button::B, mpTextB) +
                           textOffsetX + hudTransform.offsetX,
                       g_drawHIO.mButtonBFontPosY + i_textPosY + textOffsetY +
                           hudTransform.offsetY);
}
...
void dMeter2Draw_c::drawButtonZ(u8 i_action) {
    ...
    const HudItemAnchorOffset itemOffset =
        hud_button_item_offset(dusk::hud_layout::Button::Z);
    ...
    mpLightXY[2]->paneTrans(g_drawHIO.mButtonZItemBasePosX + itemOffset.x +
                                hudTransform.offsetX,
                            g_drawHIO.mButtonZItemBasePosY + itemOffset.y +
                                hudTransform.offsetY);
...
void dMeter2Draw_c::drawButtonZItem(u8 i_itemNo) {
    ...
    const auto itemPane = mpScreen->search(MULTI_CHAR('item_r_n'));
    if (i_itemNo == dItemNo_NONE_e || i_itemNo == 0) {
        ...
        itemPane->show();
        mpLightXY[2]->show();
        changeTextureItemR(i_itemNo);
        setItemParamZ(i_itemNo);
    }

    mpItemR->getPanePtr()->rotate(mpItemR->getSizeX() * 0.5f, mpItemR->getSizeY() * 0.5f,
                                  ROTATE_Z, mItemParams[SELECT_Z_e].rotation);
...
void dMeter2Draw_c::drawButtonXY(int i_no, u8 i_itemNo, u8 i_action, bool param_3, bool param_4) {
    ...
    if (!param_3) {
        ...
        const f32 hudScale = hudTransform.scale;
        if (i_no == SELECT_X_e) {
            scale_xy_hud_button_base(mpButtonXY[0], g_drawHIO.mButtonXScale * hudScale);
            mpButtonXY[0]->paneTrans(g_drawHIO.mButtonXPosX + hudTransform.offsetX,
                                     g_drawHIO.mButtonXPosY + hudTransform.offsetY);
        } else if (i_no == SELECT_Y_e) {
            scale_xy_hud_button_base(mpButtonXY[1], g_drawHIO.mButtonYScale * hudScale);
        ...
        const f32 textOffsetX = dusk::hud_layout::ButtonTextOffsetX(button);
        const f32 textOffsetY = dusk::hud_layout::ButtonTextOffsetY(button);
        ...
        mpTextXY[i_no]->paneTrans(g_drawHIO.mButtonXYTextPosX +
                                      ...
                                      textOffsetX + hudTransform.offsetX,
                                  g_drawHIO.mButtonXYTextPosY + textOffsetY +
                                      hudTransform.offsetY);
    } else {
        ...
        if (i_no == SELECT_X_e) {
            ...
            const auto hudTransform =
                dusk::hud_layout::ButtonTransform(button);
            const f32 hudScale = hudTransform.scale;
            const f32 itemScale = dusk::hud_layout::ButtonItemScale(button);
            const f32 textScale = dusk::hud_layout::ButtonTextScale(button);
            const f32 textOffsetX = dusk::hud_layout::ButtonTextOffsetX(button);
            const f32 textOffsetY = dusk::hud_layout::ButtonTextOffsetY(button);
            const HudItemAnchorOffset itemOffset = hud_button_item_offset(button);
            ...
            const HudItemAnchorOffset itemAnchorOffset = var_r29 == 0 ?
                hud_item_anchor_delta(dusk::hud_layout::Button::X,
                    g_drawHIO.mButtonXItemBasePosX[0], g_drawHIO.mButtonXItemBasePosY[0]) :
                HudItemAnchorOffset{};
            ...
            mpLightXY[0]->setAlphaRate(
                hud_button_background_alpha(mButtonXItemBaseAlpha[var_r29]) * field_0x7f0);
            ...
            const HudItemAnchorOffset itemAnchorOffset = var_r29 == 0 ?
                hud_item_anchor_delta(dusk::hud_layout::Button::Y,
                    g_drawHIO.mButtonYItemBasePosX[0], g_drawHIO.mButtonYItemBasePosY[0]) :
                HudItemAnchorOffset{};
            ...
            mpLightXY[1]->setAlphaRate(
                hud_button_background_alpha(mButtonYItemBaseAlpha[var_r29]) * field_0x7f0);
...
void dMeter2Draw_c::drawButtonCross(f32 i_posX, f32 i_posY) {
    ...
    mpButtonCrossParent->paneTrans(i_posX + hudTransform.offsetX,
                                   i_posY + hudTransform.offsetY);
}
...
void dMeter2Draw_c::setAlphaButtonChange(bool param_0) {
    ...
    if (set_parent || set_buttonXItem || param_0) {
        mpLightXY[0]->setAlphaRate(
            hud_button_background_alpha(mButtonXItemBaseAlpha[sp44[0]]) * field_0x7f0);
    }

    if (set_parent || set_buttonYItem || param_0) {
        mpLightXY[1]->setAlphaRate(
            hud_button_background_alpha(mButtonYItemBaseAlpha[sp44[1]]) * field_0x7f0);
    }

    if (set_parent || param_0) {
        mpLightXY[2]->setAlphaRate(
            hud_button_background_alpha(field_0x82c[sp44[2]]) * field_0x7f0);
    }

    if (set_parent || set_buttonZItem || param_0) {
        mpLightXY[2]->setAlphaRate(
            hud_button_background_alpha(mButtonZItemBaseAlpha) * field_0x7f0);
    }
...
void dMeter2Draw_c::setButtonIconMidonaAlpha(u32 param_0) {
    ...
    if (wiiuStyle) {
        ...
        const f32 targetY = currentDpadBottom + midnaHalfHeight + midnaTransform.offsetY;

        mpButtonMidona->scale(midnaScale, midnaScale);
        pane_trans_to_global_center(mpButtonMidona, targetX, targetY);
    } else {
        const f32 midnaScale =
            g_drawHIO.mMidnaIconScale * hudTransform.scale * itemScale * midnaTransform.scale;
        mpButtonMidona->scale(midnaScale, midnaScale);
...
void dMeter2Draw_c::setButtonIconAlpha(int i_no, u8 unused0, u32 unused1, bool unused2) {
        ...
        mpButtonXY[i_no]->isVisible())
    {
        ...
        mpLightXY[i_no]->setAlpha(hud_button_background_alpha((f32)var_r27) * temp_f31);
...
void dMeter2Draw_c::changeTextureItemR(u8 i_itemNo) {
    if (i_itemNo == dItemNo_LIGHT_ARROW_e) {
        i_itemNo = dItemNo_BOW_e;
    }
    ...
    if (dMeter2Info_readItemTexture(i_itemNo, mpItemRTex[field_0x76cR][0],
                                    (J2DPicture*)mpItemR->getPanePtr(),
                                    mpItemRTex[field_0x76cR][1], mpItemXYPane[2],
                                    NULL, NULL, NULL, NULL, -1) <= 1)
    {
        mpItemXYPane[2]->hide();
    } else {
    ...
    f32 scale;
    if (g_drawHIO.mItemScaleAdjustON == true) {
        scale = g_drawHIO.mItemScalePercent / 100.0f;
    } else {
        scale = dItem_data::getTexScale(i_itemNo) / 100.0f;
    }

    field_0x6c4[2] = scale * ((mpItemRTex[field_0x76cR][0]->width * mpItemR->getInitSizeX()) / 48.0f);
    field_0x6d0[2] =
        scale * ((mpItemRTex[field_0x76cR][0]->height * mpItemR->getInitSizeY()) / 48.0f);
    field_0x6ac[2] = (mpItemR->getInitSizeX() - field_0x6c4[2]) * 0.5f;
    field_0x6b8[2] = (mpItemR->getInitSizeY() - field_0x6d0[2]) * 0.5f;
    mpItemR->resize(field_0x6c4[2], field_0x6d0[2]);
    mpItemR->paneTrans(mItemParams[SELECT_Z_e].pos_x + field_0x6ac[2],
                       mItemParams[SELECT_Z_e].pos_y + field_0x6b8[2]);
    mpItemXYPane[2]->resize(field_0x6c4[2], field_0x6d0[2]);
}
...
void dMeter2Draw_c::drawKanteraMeter(u8 i_button, f32 i_alphaRate) {
    ...
    CPaneMgr* pane = i_button == SELECT_Z_e ? mpItemR : mpItemXY[i_button];
    f32 sp10[SELECT_ITEM_MAX_e] = {0.0f};
    f32 sp8[SELECT_ITEM_MAX_e] = {0.0f};
...
void dMeter2Draw_c::setItemParamX(u8 i_itemNo) {
    ...
    apply_hud_item_anchor(dusk::hud_layout::Button::X, g_drawHIO.mButtonXItemBasePosX[0],
                          g_drawHIO.mButtonXItemBasePosY[0], mItemParams[SELECT_X_e].pos_x,
                          mItemParams[SELECT_X_e].pos_y);
}
...
void dMeter2Draw_c::setItemParamY(u8 i_itemNo) {
    ...
    apply_hud_item_anchor(dusk::hud_layout::Button::Y, g_drawHIO.mButtonYItemBasePosX[0],
                          g_drawHIO.mButtonYItemBasePosY[0], mItemParams[SELECT_Y_e].pos_x,
                          mItemParams[SELECT_Y_e].pos_y);
}
