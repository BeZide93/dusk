...
bool hud_button_uses_round_style(dusk::hud_layout::Button button) {
    return (dusk::hud_layout::ButtonStyleFlags(button) & DUSK_MOD_HUD_BUTTON_STYLE_ROUND) != 0;
}

void scale_xy_hud_button(CPaneMgr* button, dusk::hud_layout::Button hudButton, f32 scale) {
    if (!hud_button_uses_round_style(hudButton)) {
...
f32 hud_backing_alpha(f32 alpha) {
    return dusk::hud_layout::ElementVisible(dusk::hud_layout::Element::ButtonBackground) ? alpha :
                                                                                           0.0f;
}
...
void dMeter2Draw_c::initButton() {
    ...
    if (hud_button_uses_round_style(dusk::hud_layout::Button::X)) {
        make_hud_button_round(mpLightXY[0], mpButtonA, true);
        make_hud_button_round(mpButtonXY[0], mpButtonA, false);
    }
    if (hud_button_uses_round_style(dusk::hud_layout::Button::Y)) {
...
void dMeter2Draw_c::drawButtonXY(int i_no, u8 i_itemNo, u8 i_action, bool param_3, bool param_4) {
    ...
    if (!param_3) {
        ...
        const f32 hudScale = hudTransform.scale;
        if (i_no == SELECT_X_e) {
            scale_xy_hud_button(mpButtonXY[0], button, g_drawHIO.mButtonXScale * hudScale);
            mpButtonXY[0]->paneTrans(g_drawHIO.mButtonXPosX + hudTransform.offsetX,
                                     g_drawHIO.mButtonXPosY + hudTransform.offsetY);
        } else if (i_no == SELECT_Y_e) {
            scale_xy_hud_button(mpButtonXY[1], button, g_drawHIO.mButtonYScale * hudScale);
