...
#include "dusk/mods/svc/combat_input.hpp"
#include "dusk/mods/svc/item_aim.hpp"
...
static bool daAlink_hasZItemSlot() {
    return dusk::mods::svc::item_slots::z_slot_enabled() != 0;
}

static bool daAlink_manualShieldingEnabled() {
    return dusk::mods::svc::combat_input::manual_shielding_enabled() != 0;
}

static bool daAlink_manualShieldButton(daAlink_c* i_link) {
    return daAlink_manualShieldingEnabled() &&
           dusk::mods::svc::combat_input::manual_shield_button(i_link) != 0;
}

static u8 daAlink_selectItemButtonCount() {
    return daAlink_hasZItemSlot() ? 3 : 2;
}
...
static int daAlink_meterUseButtonForSelectItem(u8 i_idx) {
    ...
    return i_idx < 3 ? buttons[i_idx] : 0;
}

static bool daAlink_isLegacyZSelectItemSlot(int i_slot) {
    return i_slot == SELECT_ITEM_DOWN && !daAlink_hasZItemSlot();
}
...
BOOL daAlink_c::swordSwingTrigger() {
    if (daAlink_manualShieldButton(this)) {
        return false;
    }
...
BOOL daAlink_c::checkItemAction() {
    ...
    if (checkWolf()) {
        ...
        {
            if (daAlink_manualShieldingEnabled()) {
                if (daAlink_manualShieldButton(this)) {
...
#endif
    {
...

        {
            ...
            for (int i = 0; i < daAlink_selectItemButtonCount(); i++) {
                ...
                } else if (i == SELECT_ITEM_DOWN && daAlink_hasZItemSlot()) {
                    dMeter2Info_onUseButton(METER2_USEBUTTON_Z);
                }
            }
